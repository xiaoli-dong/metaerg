...
var x = d3.scalePow().range([0, 2 * Math.PI]);
var y = d3.scalePow().range([0, 1.3* radius]);
...
function createVisualization(json) {
 ...
 // Get total size of the tree = value of root node from partition.
	...
	.attr("dx", function(d) {
	    ...
	    var mytext =   d.data.text;
	    //return (mytext.replace(/^FIG\S+?:/, "")).substring(0, 30);
	    if(d.depth ==1){
	    return d.data.text;
		}
        });
...
function mouseover(d) {
    ...
    tooltip.text(d.data.text + " " + percentageString)
        .style("opacity", 0.8)
